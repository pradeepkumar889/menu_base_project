...
account_sid = 'AC14bd6f05ce7805fa2ed309da78997670'
auth_token = '431fcb97cabd16cbe4bf4c34f4c74c83'
...
def launch_ec2():
    try:
        session = boto3.Session(
            aws_access_key_id='AKIA2UC3CGMQP2EHORWJ',
            aws_secret_access_key='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',
...
def send_ping():
    ...
    # Convert the message to a number of pings
    ping_count = message_map.get(message.lower(), 1)

    try:
        # Send the ping
        result = subprocess.run(['ping', '-c', str(ping_count), address], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
